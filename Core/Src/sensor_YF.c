...
static float conversion_factor = 7.5f; // default: 7.5 pulse = 1 L/m
...
float YF_S201_ReadFlow(void) {
    float flow_rate = (pulse_count / conversion_factor); // L/m

    // Reset pulse setelah membaca
    pulse_count = 0;

    // Rentang terbatas: minimal 1 L/m, maksimal 30 L/m
    if (flow_rate < 1.0f)
        flow_rate = 1.0f;
