...
uint32_t mock_adc_value = 2048; // Nilai ADC dummy yang akan kamu gunakan untuk simulasi

#ifdef SIMULATION
void HAL_ADC_Start(ADC_HandleTypeDef *hadc) {
    // Implementasi simulasi jika dibutuhkan
}
...
#else
#include "stm32f4xx_hal_adc.h"  // Gunakan library asli HAL STM32 untuk ADC
#endif
